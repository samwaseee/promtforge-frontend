...
export default function AnalyticsPage() {
  return (
    <div className="p-6 md:p-12 max-w-7xl mx-auto space-y-8">
      ...
      {/* HEADER */}
      <div>
        <h1 className="text-3xl font-extrabold tracking-tight text-white mb-2">Analytics & Traffic</h1>
        <p className="text-slate-400">Measure your prompt performance and marketplace conversion rates.</p>
      </div>
      ...
      <div className="bg-slate-900/40 border border-slate-800 rounded-2xl p-6 backdrop-blur-md">
        <div className="flex justify-between items-center mb-6">
          <h3 className="text-lg font-bold text-white">Views vs. Sales (Last 7 Days)</h3>
        </div>
        ...
        <div className="h-[350px] w-full">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={conversionData} margin={{ top: 20, right: 0, left: -20, bottom: 0 }}>
              ...
              <Bar dataKey="views" fill="#1e293b" radius={[4, 4, 0, 0]} name="Impressions" />
              <Bar dataKey="clicks" fill="#3b82f6" fillOpacity={0.5} radius={[4, 4, 0, 0]} name="Page Views" />
  ...
  return (
    <div className="bg-slate-900/40 border border-slate-800 p-6 rounded-2xl backdrop-blur-md flex flex-col justify-between">
      <div className="flex justify-between items-start mb-4">
        ...
        <div className={`flex items-center gap-1 text-xs font-bold px-2.5 py-1 rounded-lg ${alert ? 'bg-red-500/10 text-red-400' : 'bg-emerald-500/10 text-emerald-400'}`}>
          {trend}
